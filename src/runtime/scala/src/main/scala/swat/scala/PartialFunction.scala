...
object PartialFunction {
   ...
   */
  private[this] val fallback_pf: scala.PartialFunction[Any, Any] = { case _ => fallback_pf }
  ...
  private class Unlifted[A, B] (f: A => Option[B]) extends swat.scala.runtime.AbstractPartialFunction[A, B] {
   ...
   *  @since   2.10
   */
  def apply[A, B](f: A => B): scala.PartialFunction[A, B] = { case x => f(x) }
